...
(*let file = Queue.create ();;*) (*"File" used for the algorithm*)
...
(*Creates path after bellmanford*)
...
(*Bellmanford variant algorithm, updating cost of arcs in correlation with the successors*)
let bellmanford gr id1 id2 =
  let file = Queue.create() (*Creates "File"*)
  and init_list = n_fold gr (fun liste id -> (id,Int.max_int,-1)::liste) [] in
  let first_list = replace init_list id1 (id1,0,-1) in (*Label of origin changed*)
  Queue.add id1 file;
  let rec loop_file l_label k =
    (*Condition to stop algorithm if infinite loop : if (k>10) then failwith "Stop algo" else*)
    match Queue.take_opt file with
    | None -> printf "File vide! Fin de l'algorithme.\n\n\n%!";l_label
    | Some elt -> printf "Element extrait de la file : %d \n%!" elt;print_list_label l_label;
    (*Loop on current elemnt's successors*)
    ...
    | (id_succ1,len)::rest -> printf "Id successeur : %d et cout arc : %d\n%!" id_succ1 len;
      ...
      if (cost_pred+len)<cost_succ then 
        ...
        (*If element already in queue, don't add it again*)
